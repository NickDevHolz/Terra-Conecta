...
import type { MarketStep } from '../market.types';

export function MarketStepGrid({
  steps,
  selectedId,
}: {
  steps: MarketStep[];
  ...
  return (
    <section className="rounded-[30px] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.88),rgba(255,248,236,0.78))] p-4 shadow-[0_12px_30px_rgba(91,64,37,0.05)]">
          ...
          return (
            <button
              ...
              className={`min-h-[130px] rounded-[26px] border-2 p-4 text-center shadow-[0_8px_18px_rgba(91,64,37,0.04)] transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.99] ${
                active ? 'border-[#C88919]' : 'border-white/80'
              } ${step.id === 'renda' ? 'col-span-2' : ''}`}
